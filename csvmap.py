	...
	def __init__(self):
		...
		for x in np.linspace(-self.x_dim / 2, self.x_dim / 2, self.n_sin):
			for position in (-1, 1):
				...
				for y in np.linspace(-self.y_dim / 2, self.y_dim / 2, self.n):
					if (y > line and position == 1) or (y < line and position == -1):
						self.rectangles.append(
							Rectangle(
								x_center=x,
								y_center=y,
		...
		for i, x in enumerate(X):
			for j, y in enumerate(Y):
				...
				if not x_within_bounds or not y_within_bounds:
					out[len(Y) - j - 1][i] = self.wall_height
				...
				self.map[counter] = [x, y, out[len(Y) - j - 1][i]]
				counter += 1
		return out
